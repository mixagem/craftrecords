...
function mixagemAudioPlayer() {
  ...
  function playlistEntryInfo() {
    ...
    let larguraPlayerOffset = playlistMainTable.clientHeight * .8;                // *.7 serve para tornar a thumb mais quadrada
    ...
    (function () {
      ...
      document.querySelector('#playlist-table').style.width = `${larguraPlayer - alturaPlayer}px`;
    })();
  ...
  function albumArtwork() {
    albumThumbnail.style.width = `${alturaPlayer}px`;
  }
  ...
  function nextTrack() {
    ...
    if (audioIDIndex <= trackCount) {
      audioID.src = `assets-mp3/mp3/audio${audioIDIndex}.mp3`;
      removeNowPlayingStyle();
      nowPlayingStyle();
      changeArtwork(audioIDIndex);
      return audioID
    } else {
      audioID.src = `assets-mp3/mp3/audio1.mp3`;
      removeNowPlayingStyle();
      nowPlayingStyle();
      changeArtwork(1);
      return audioID
  ...
  function trackSelect(e) {
    ...
    audioID.src = `assets-mp3/mp3/audio${gridClassNameIndex}.mp3`;
    removeNowPlayingStyle();
    nowPlayingStyle();
    stop();
    playpause();
  ...
  function playpauseArtwork() {
    if (!audioID.paused) {
      artworkClick.innerHTML = '<i class="far fa-pause-circle"></i>';
    }                                              // se tiver em pausa, mete play
    else {
      artworkClick.innerHTML = '<i class="far fa-play-circle"></i>';
  ...
  function changeArtwork(i) {
    if (i === undefined) {
      ...
      albumThumbnail.style.backgroundImage = `url('assets-mp3/img/thumb${indexForThumb}.jpg')`;
    }
    else {
      albumThumbnail.style.backgroundImage = `url('assets-mp3/img/thumb${i}.jpg')`;
    }
  }
